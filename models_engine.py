...
NUMBERS_COUNT = 56
DRAW_SIZE = 6
NUMBERS = np.arange(1, NUMBERS_COUNT + 1)
...
def transformer_model(
    ...
    if len(window) == 0:
        weights = np.full(NUMBERS_COUNT, 1 / NUMBERS_COUNT)
    else:
        torch.manual_seed(int(rng.integers(0, 1_000_000)))
        attn = torch.nn.MultiheadAttention(
            embed_dim=NUMBERS_COUNT, num_heads=4, batch_first=True
        )
        with torch.no_grad():
...
def lstm_model(
    ...
    if len(one_hot) == 0:
        ...
        torch.manual_seed(int(rng.integers(0, 1_000_000)))
        lstm = torch.nn.LSTM(
            input_size=NUMBERS_COUNT,
            hidden_size=64,
            num_layers=1,
            batch_first=True,
        )
        projection = torch.nn.Linear(64, NUMBERS_COUNT)
        with torch.no_grad():
...
def montecarlo_model(
    ...
    for start in range(0, iterations, chunk_size):
        ...
        random_matrix = rng.random((size, NUMBERS_COUNT))
...
def mean_reversion_model(
    ...
    expected = (len(draws) * DRAW_SIZE) / NUMBERS_COUNT if len(draws) else 1.0
...
def gnn_model(
    ...
    if np.all(adjacency == 0):
        ...
        adj = adj + torch.eye(NUMBERS_COUNT)
        degree_inv = torch.diag(
            1.0 / torch.clamp(adj.sum(dim=1), min=MIN_DEGREE_SUM)
        )
        norm_adj = degree_inv @ adj
        features = torch.eye(NUMBERS_COUNT)
        weight = torch.randn(NUMBERS_COUNT, 16)
...
def umap_model(
    ...
    for cluster_id in range(n_clusters):
        ...
        top_indices = np.argsort(cluster_freq)[-UMAP_CHOICE_POOL_SIZE:]
        choice_pool = top_indices + 1
        restricted_weights = normalize_weights(cluster_freq[top_indices])
        for _ in range(10):
            numbers = rng.choice(
                choice_pool, size=DRAW_SIZE, replace=False, p=restricted_weights
            )
            numbers.sort()
...
def _generate_sequences_from_weights(
    ...
    while len(sequences) < count and attempts < count * MAX_ATTEMPTS_MULTIPLIER:
        ...
        if attempts - last_success > MAX_ATTEMPTS_MULTIPLIER:
            break
        numbers = rng.choice(NUMBERS, size=DRAW_SIZE, replace=False, p=weights)
        numbers.sort()
        combo = tuple(numbers.tolist())
        if combo in forbidden or combo in seen:
            continue
...
def _frequency_vector(draws: np.ndarray) -> np.ndarray:
    if draws.size == 0:
        return np.ones(NUMBERS_COUNT)
    counts = np.bincount(draws.flatten(), minlength=NUMBERS_COUNT + 1)[1:]
    return counts.astype(float)


def _one_hot_draws(draws: np.ndarray) -> np.ndarray:
    if draws.size == 0:
        return np.empty((0, NUMBERS_COUNT), dtype=np.float32)
    one_hot = np.zeros((len(draws), NUMBERS_COUNT), dtype=np.float32)
...
def _cooccurrence_matrix(draws: np.ndarray) -> np.ndarray:
    matrix = np.zeros((NUMBERS_COUNT, NUMBERS_COUNT), dtype=float)
    for draw in draws:
        indices = draw - 1
...
def _crossover(
    ...
    if len(pool) >= 6:
        child = rng.choice(pool, size=DRAW_SIZE, replace=False)
    else:
        remaining = np.setdiff1d(NUMBERS, pool)
        fill = rng.choice(remaining, size=DRAW_SIZE - len(pool), replace=False)
